...
const TICK_RATE = 2000; // Speed of the game
...
const NIGHT_LENGTH = 3; // Tip: Result number btw 0 and 2 + 5
...
const gameSate = {
  ...
  tick() {
    ...
    if (this.clock === this.wakeTime) this.wake();else if (this.clock === this.sleepTime) this.sleep();else if (this.clock === this.hungryTime) this.hungryTime();else if (this.clock === this.dieTime) this.dieTime();else if (this.clock === this.timeToStartCelebrating) this.startCelebrating();else if (this.clock === this.endCelebrating) this.endCelebrating();else if (this.clock === this.poopTime) this.poop();
    return this.clock;
  },

  startGame() {
    console.log("hatching");
    this.current = "HATCHING"; // Awake for 3 sec, 9 sec to hatch
  ...
  wake() {
    console.log("awoken");
    this.current = "IDLING"; // He's no longer waiting to wake up

    this.wakeTime = -1; // 1 rain : 0 day
  ...
  die() {
    ...
    (0, _ui.modScene)("dead");
    this.clearTimes();
    (0, _ui.writeModal)("He dies... Press the middle to start");
  },
  ...
  handleUserAction(icon) {
    console.log(icon);
    if (["SLEEP", "FEEDING", "CELEBRATING", "HATCHING"].includes(this.current)) return; // do nothing
  ...
  feed() {
    ...
    (0, _ui.modFox)("eating"); // Finish eating & begin celebrating
...
const handleUserAction = gameSate.handleUserAction.bind(gameSate);
...
function initButtons(handleUserAction) {
  ...
  let selectedIcon = 0; // target is gonna be the actual DOM node that I've clicked on

  function buttonClick({
    target
  }) {
    if (target.classList.contains("left-btn")) {
      console.log("left button"); // Disable the current one

      toggleHighlighted(selectedIcon, false); // tip: 10 % 3 = 1

      selectedIcon = (2 + selectedIcon) % _constants.ICONS.length; // Active the new one
...
// bound version of handleUserAction
async function init() {
  console.log("starting game"); // tip: initButton(console.log(icon))
